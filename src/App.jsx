...
function App() {
  useEffect(() => {
      ...
      loadContent: async () => {
        try {
          ...
          if (docSnap.exists()) {
            ...
            console.log(
              "No content found in Firestore. Seeding with default data."
            );
    ...
    function SudoManager() {
      const authenticate = async () => {
        const enteredPassword = window.prompt("Enter administrator password:");
        ...
        try {
          ...
          if (adminDocSnap.exists()) {
            ...
            if (newPassword) {
              await setDoc(contentStore.adminDocRef, {
      ...
      const actions = {
        ...
        add: {
          projects: () => {
            ...
            if (!name) return;
            const description = window.prompt("Enter project description:");
            if (description === null) return;
            const link = window.prompt("Enter project link (URL):", "https://");
            if (link === null) return;
          ...
          skills: () => {
            const category = window.prompt(
              "Enter skill category (e.g., Languages):"
            );
            if (!category) return;
            const items = window.prompt("Enter skills (comma-separated):");
          ...
          experience: () => {
            ...
            const period = window.prompt("Enter period (e.g., Summer 2024):");
        ...
        edit: (section) => {
          ...
          for (const key in item) {
            const newValue = window.prompt(`Edit '${key}':`, item[key]);
    ...
    function init3D() {
      ...
      // ✅ Make renderer output in sRGB (so it matches how the browser shows your image)
      ...
      profileImage.onload = () => {
        // background just in case there are bars – doesn’t change pixels where the image is
        ctx.fillStyle = "black";
        ...
        // ❗ No filters, no darkening, no lightening – draw raw image
      ...
      const materials = [
        ...
        new THREE.MeshBasicMaterial({ color: 0x161b22 }),
        new THREE.MeshBasicMaterial({ color: 0x161b22 }),
        new THREE.MeshBasicMaterial({ map: texture }), // front (your photo)
  ...
  return (
    <div className="min-h-screen bg-[#0d1117]">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 h-screen p-4 md:p-8">
        ...
        <div className="lg:col-span-2 h-full">
          <div
            id="terminal"
            className="terminal rounded-lg shadow-xl p-4 h-full overflow-y-auto flex flex-col"
          >
